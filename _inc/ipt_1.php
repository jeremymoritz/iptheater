...
// $previousShow = new Show(2, 'Bye Bye Birdie', 'BBB');
$previousShow = new Show(34, '42nd Street', 's42');
$currentShow = new Show(26, 'High School Musical', 'hsm');
$sql = "
	SELECT
		id,
		dateTime
...
if ($developerMode) {
	...
	$currentShow = $currentShow ? $currentShow : new Show(34, '42nd Street DEVELOPER', 's42');
	...
	$sth = $dbh->prepare($sql);
	$sth->execute();
	$performances = sthFetchObjects($sth);	//	fetch all of the seats
	foreach($performances as $p) {
		$currentShow->addPerformance(new Performance($p->id, $p->dateTime));
	}

	//	change the performances at will instead of relying on the DB
	// $currentShow->addPerformance(new Performance(1, '2014-03-15 12:00:00'));
	// $currentShow->addPerformance(new Performance(2, '2014-03-15 17:00:00'));
	// $currentShow->addPerformance(new Performance(3, '2014-03-16 17:00:00'));
