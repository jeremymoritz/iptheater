...
if(apiPost('submit') && !$likelySpam) {
	...
	$sql = "
		SELECT c.id, c.role, c.group1, c.group2, group3, p.dateOfBirth, p.email
		FROM ip_cast c
	...
	if ($castId) {
		...
		$autoApproved = ($castBioIdExists || strtotime('2014-12-30') < strtotime('now')) ? 0 : 1;	//	auto-approve this if this is a first-time bio for this cast member and it's before Mindy's birthday
...
<?php
	if(apiGet('formSubmitted')) {
		...
		echo "<h3>Oops!  Your name is not found in our <a href='cast_list.php' target='_blank'>cast list</a>!</h3>
			<p>Please click the back button to check the spelling and capitalization of your name to ensure that it matches our cast list.  If you feel you've reached this message in error, please copy/paste your bio in an email and send it to " . disguiseMailLink($config['info_email']) . ".</p>";
	} else {
?>
		<h3>Bio Form for <?=$currentShow->getTitle();?></h3>
		<p>Please complete this form in order for your bio to be included in our <a href='bios.php' class='btn btn-success'>cast bios page</a>!</p>
		<hr>
				...
				<section class='col-sm-6'>
					...
					<div class='row top-buffer'>
						<section class='col-xs-12'>
							<textarea name='bio' rows='12' class='form-control' placeholder='Your bio goes here' data-bind='value: bio, valueUpdate: "keypress"'></textarea>
						</section>
					</div>
				</section>
				<section class='col-sm-6'>
