...
if(apiPost('receiver_id') === $config['paypal_code']) {
	...
	if(floatval($custom['orderPrice']) === floatval(apiPost('payment_gross'))) {
		...
		$message = "<b>name:</b> {$order->name}<br>\n"
			...
			. "<b>performance:</b> {$currentShow->getPerformance($custom['performanceId'])->getDateTimeFormatted()}<br>\n"
		...
		$message = "
			...
			<p>Purchased: {$order->ticketCount} Ticket" . ($order->ticketCount > 1 ? "s" : "") . " to {$currentShow->getTitle()} at {$currentShow->getPerformance($custom['performanceId'])->getDateTimeFormatted()} at The Goppert Theater at Avila University (11901 Wornall Rd, Kansas City, MO 64145)</p>
