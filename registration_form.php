...
if(apiPost('submit') && !$likelySpam) {
	...
	$message = "<b>name:</b> $firstName $lastName<br>\n"
		...
		. "<b>ip:</b> $ip<br>\n";
	$subject = "13 Registration: $firstName $lastName";
	...
	header("Location: pay.php?amount=50&name={$parentName}&email={$parentEmail}&phone={$parentPhone}&comments=13%20Holding%20Fee:%20{$firstName}%20{$lastName}");
}
...
<?=$header;?>
	<section class='main'>
		...
		<h2><img src='_img/ip-logo-long_400_trans.png' alt='Immeasurable Productions'><br>
			<img src='_img/thirteen-logo_400.png' alt='13' id='thirteen_logo'>
		</h2>
		<h3>Registration Form for 13</h3>
		...
		<form action='<?=$_SERVER['PHP_SELF'];?>' method='post' role='form' class='bs-condensed form-inline'>
			...
			<div class='row'>
				<section class='col-sm-12 text-center top-buffer'>
					<input type='submit' name='submit' value='Submit Registration Form And Pay $50 Holding Fee' onclick='return confirm("Is the form complete and accurate?")' class='btn btn-primary'>
				</section>
			</div>
		</form>
	</section>
