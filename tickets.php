...
$page = apiPost('p', 1);
$mailingFee = 2;
$hoursBeforeShowtime = 3;	//		tickets may be orderd online up until 3 hours before showtime
...
$serviceCharge = $ticketBoothMode ? 0 : 3;
...
switch($page) {
		...
		foreach($currentShow->getPerformances() as $p) {
			if($ticketBoothMode) {
				...
				if(date('Y-m-d H:i') <= date('Y-m-d H:i', strtotime($p->getDateTime() . " +1 hour"))) {
					...
					$pageContent .= "
								<li class='list-group-item'><label><input type='radio' name='performanceId' value='{$p->getId()}' required>{$p->getDateTimeFormatted()}</label></li>";
	...
	case 2:
		...
		$sql = "
			SELECT
				...
				s.price
			FROM ip_seats s
			" . (count($unavailableSeats) ? "WHERE s.id NOT IN (" . implode(", ", $unavailableSeats) . ")" : "");
		$sth = $dbh->prepare($sql);
		$sth->execute();
	...
	case 4:
		...
		if (in_array(strtoupper(apiSession('referredBy')), array('ULX', 'UNITEDLEX', 'UNITED LEX'))) {	//	SPECIAL! Free Tickets for UnitedLex employees!
			$totalTicketCost = 0;
			$serviceCharge = 1;
		}
